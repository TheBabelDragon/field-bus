...
/**
 * Field Bus — 29-bit extended CAN ID helpers
 *
 * Layout:
 *   bits 28..26  Priority   (0 = highest)
 *   bits 25..18  Msg Type
 *   bits 17..10  Source node
 *   bits  9.. 2  Target node (0 = broadcast)
 *   bits  1.. 0  Flags / reserved
 */
...
/* Well-known node IDs (suggestions, not mandatory) */
...
/* Convenience constructors for common classes */
static inline uint32_t fb_id_emergency(uint8_t src, uint8_t dst) {
    return fb_make_id(FB_PRIO_EMERGENCY, FB_MSG_EMERGENCY_STOP, src, dst);
}
